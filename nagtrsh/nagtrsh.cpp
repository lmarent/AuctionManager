...
/*! \file nagtrsh.cpp
    ...
    nagtrsh is the NetAgent remote shell
    
    this tool can be used for remote control of NetAgent managers
    it supports all command which are part of the agent control interface
    

    $Id: nagtrsh.cpp 2015-08-26 13:18:00 amarentes $
...
int parseCommand(string input, string &action, string &postfields)
{
    ...
    } else if (action == "rm_req") {
        action = "/rm_req";
        input_str >> param;
        if (!param.empty()) {
            postfields = "ResourceRequestID=" + param;
        }
    } else if (action == "add_req") {
        action = "/add_req";
        getline(input_str, param);
        if (!param.empty()) {
            postfields = "ResourceRequest=" + param;
        }
    } else if (action == "add_reqs") {
        action = "/add_req";
        input_str >> param;
...
int main( int argc, char *argv[] )
{
  ...
  args.add('s', "NetAgentServer", "<server>", "select netAgent server"
           , "MAIN", "server");
  args.add('p', "NetAgentPort", "<port>", "select netAgent port", "MAIN", "port");
  ...
  if (args.getArgValue('V') == "yes") {
      ...
      cout << "nagtrsh version: " << VERSION << endl;
      exit(0);
  }
