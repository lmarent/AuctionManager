...
AUMSession *SessionManager::getSession(int uid)
{
...
AUMSession *SessionManager::getSession(string _sessionId)
{
...
void 
SessionManager::addSession(AUMSession *session)
{
...
string SessionManager::getInfo(AUMSession *s)
{
    ostringstream os;
...
void SessionManager::delSession(AUMSession *s, EventScheduler *e)
{
#ifdef DEBUG    
    log->dlog(ch, "removing Session with Id = '%s'", s->getSessionId().c_str());
#endif

	if( EventSchedulerAuctioner * eauct = dynamic_cast<EventSchedulerAuctioner*>(e) )
	{
		// remove Session from database and from index
		storeAUMsessionAsDone(s);
		sessionDB[s->getUId()] = NULL;
		sessionIndex.erase(s->getSessionId());
		
		if (eauct != NULL) {
			eauct->delSessionEvents(s->getUId());
		}

		sessions--;
	}
	else {
		throw Error("Event scheduler given is not of type Auctioner");
	}
}
...
void SessionManager::storeSessionAsDone(AUMSession *s)
{
