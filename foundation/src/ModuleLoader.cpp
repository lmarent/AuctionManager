...
#ifdef DEBUG
        s_log->log(ch, "Filename to load '%s'", filename.c_str());
#endif
...
#ifdef DEBUG
			s_log->log(ch, "Filename failed '%s'",  dlerror());
#endif			
			fprintf(stderr, "%s\n", dlerror());
		}
...
#ifdef DEBUG
			s_log->log(ch, "dlopen  Ok _ magic Number: %d - PRO_MAGIC:%d", magic, PROC_MAGIC );
#endif	            
